...
export class VentureDescriptionGenerator {
  ...
  getIPNarrative() {
    ...
    if (ip >= 6) {
      return "Intellectual property protection is well-established with active patent prosecution and FTO assessment completed. ";
    }
    if (ip >= 3) {
      return "The company has made progress on IP protection with invention disclosures filed and initial patent applications submitted. ";
    }
  ...
  getTeamNarrative() {
    ...
    if (team >= 6) {
      return "The team is well-developed with experienced leadership and strong execution capabilities. ";
    }
    if (team >= 3) {
      return `The founding team is ${team >= 4 ? "building functional departments" : "making key early hires"} and ${team >= 5 ? "has engaged advisory support" : "recognizes capability gaps"}. `;
    }
  ...
  getBusinessFundingNarrative() {
    ...
    if (biz >= 4 && funding >= 4) {
      ...
      if (funding >= 8) {
        ...
        fundingStatus = "has a complete data room and professional valuation";
      } else if (funding >= 5) {
        fundingStatus = "has all due diligence materials prepared";
      } else {
        fundingStatus = "is pitch-ready with complete fundraising materials and strategy";
      }
  ...
  getRegulatoryNarrative() {
    ...
    if (reg >= 6) {
      return "Regulatory progress is advanced with submissions to agencies underway or completed. ";
    }
    if (reg >= 3) {
