...
import { readinessData, EXPORT_ATTRIBUTION_LINES } from "./data/index.js";
import {
...
export function savePdfSnapshot(options = {}) {
  ...
  // Footer
  y = ensurePdfSpace(doc, y + lh, 12 + EXPORT_ATTRIBUTION_LINES.length * 4);
  doc.setFontSize(10);
  doc.setTextColor(100);
  doc.text("Generated by NobleReach Readiness Level Assessment Tool (Pilot v0.5)", left, y);

  // Licensing and source attribution. The PDF leaves our control, so the terms
  // that govern downstream reuse of the BRL content travel with it.
  y += lh - 1;
  doc.setFontSize(7);
  doc.setTextColor(130);
  EXPORT_ATTRIBUTION_LINES.forEach((line) => {
