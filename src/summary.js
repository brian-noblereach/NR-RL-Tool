...
// Updated April 2026 - Aligned with revised definitions
...
export class VentureDescriptionGenerator {
  ...
  getTechnologyProductNarrative() {
    ...
    if (tech >= 5 && product >= 4) {
      ...
      const productStatus = product >= 6
        ? "product processes and pricing strategy in place"
        : product >= 4
          ? "a working prototype deployed for user feedback"
  ...
  getMarketGTMNarrative() {
    ...
    if (market >= 5 && gtm >= 5) {
      const marketStatus = market >= 7
        ? "ongoing customer engagement driving strategy decisions"
        : "active engagement confirming willingness to pay";
      const gtmStatus = gtm >= 7
        ? "is generating consistent leads and pipeline"
        : "has a defined strategy aligning product, marketing, and sales";
      narrative = `Market validation is strong with ${marketStatus}, while go-to-market execution ${gtmStatus}. `;
    } else if (market >= 3 || gtm >= 3) {
      narrative = `The company has ${market >= 4 ? "completed customer discovery validating the target problem" : market >= 3 ? "defined competitive positioning with clear differentiation" : "initiated market research"}, ${gtm >= 3 ? "and has a buyer journey hypothesis defined" : "but go-to-market strategy remains underdeveloped"}. `;
    } else {
  ...
  getTeamNarrative() {
    ...
    if (team >= 6) {
      return "The team has all key competencies for the current stage with mentors and advisors actively supporting the venture. ";
    }
    if (team >= 4) {
      return `The team has ${team >= 5 ? "the founding team aligned with ownership and roles agreed" : "engaged advisors and mentors"} and is building organizational capability. `;
    }
    if (team >= 2) {
  ...
  getBusinessFundingNarrative() {
    ...
    if (biz >= 4 && funding >= 4) {
      const bizStatus = biz >= 6
        ? "initial revenue aligned to the business model"
        : biz >= 5
          ? "a verified business model through commercial engagement"
          : "a validated business plan with pricing tested";
      ...
      narrative = `Business planning ${biz >= 3 ? "includes an initial strategic business plan with milestones defined" : "has outlined the business model"}, while ${funding >= 3 ? "financial modeling and cap table are documented" : funding >= 2 ? "the legal entity is properly established" : "funding preparation is beginning"}. `;
    }
